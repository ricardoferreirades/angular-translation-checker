...
function extractDynamicPatterns(content) {
  ...
  // Pattern 3: Template literals in TypeScript - translate.get(`prefix.${var}.suffix`) and translateService variants
  // Enhanced to handle function calls like toScreamingSnakeCase() and multiline patterns
  const tsTemplateRegex = /(?:translate|translateService)\.(get|instant)\(\s*[`]([\w._-]*(?:\$\{[^}]+\}[\w._-]*)*)[`]/gs;
  ...
  // Pattern 4: String concatenation in TypeScript - translate.get('prefix.' + var + '.suffix') and translateService variants
  const tsConcatenationRegex = /(?:translate|translateService)\.(get|instant)\(\s*['"`]([\w._-]*)['"]\s*\+\s*[^+'"]+\s*(?:\+\s*['"`]([\w._-]*)['"]\s*)?\)/gs;
