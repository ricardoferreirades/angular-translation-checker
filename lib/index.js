...
function extractDynamicPatterns(content) {
  ...
  // Enhanced to handle function calls like toScreamingSnakeCase()
  const tsTemplateRegex = /translate\.(get|instant)\(\s*[`]([\w._-]*(?:\$\{[^}]+\}[\w._-]*)*)[`]\s*\)/g;
  ...
  const tsConcatenationRegex = /translate\.(get|instant)\(\s*['"`]([\w._-]*)['"]\s*\+\s*[^+'"]+\s*(?:\+\s*['"`]([\w._-]*)['"]\s*)?\)/g;
